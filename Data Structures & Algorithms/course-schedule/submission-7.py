        ...
        def dfs(course):
            ...
            for nei in adj.get(course,[]):
                ...
                if not dfs(nei):
                    ...
                    return False
                visited.remove(nei)# after we are done with this path, backtrack
            taken.add(course) # course is fully explored and safe (no cycle)
            return True

        for i in range(numCourses):
            visited = set() # used for cycle detection; courses on the current DFS path
            visited.add(i) # add the current course
            res = dfs(i)
