class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        for course, preq in prerequisites:
            ...
            if course not in courseToprerequisite:
                courseToprerequisite[course] = []
            courseToprerequisite[course].append(preq)
        courseCount = {} # number of prerequisites for each course
        for course in range(numCourses):
            courseCount[course] = 0
            if course in courseToprerequisite:
                courseCount[course] = len(courseToprerequisite[course])
        finish = 0
        q = deque()
        for course in range(numCourses):
