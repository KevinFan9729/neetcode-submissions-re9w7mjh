class Solution:
    def makesquare(self, matchsticks: List[int]) -> bool:
        ...
        # at every step, at index i, what side should i goes to?
        # Time O(4^n) # each steps have 4 decision
        # Space O(n)
        ...
        def square(i):
            ...
            if count == 4:
                return True
            # prune, for the current stack, do not try placing stick to already seen equivalent side
            # say we have [0,0,0,0] and stick is 1, placing the 1 to location 0 is enough
            seen = set() 
            for pos in range(4):
                stick = matchsticks[i]
