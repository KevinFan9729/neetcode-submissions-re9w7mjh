class Solution:
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        # you can also treat the entire matrix as a 1d sorted array
            # the mapping of values are:
                # m, n = len(matrix), len(matrix[0]) , m is row and n is col
                # left, right = 0, m * n - 1
                # row = mid // n
                # col = mid % n
                # val = matrix[row][col]
        # Time O(log(m) + log(n)) = log(m*n)
        # Space O(1)
