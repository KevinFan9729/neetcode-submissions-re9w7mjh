class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        # O(n*sum(nums)) (index, currSum) unique states are computed nonrepeatedly, index is bounded by n (len(nums)), currSum is bounded -sum(nums) to sum(nums)
        # O(n*sum(nums)) in space, the hash map needs to record all our states
