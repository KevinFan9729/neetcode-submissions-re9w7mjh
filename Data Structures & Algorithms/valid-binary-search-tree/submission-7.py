...
class Solution:
    def isValidBST(self, root: Optional[TreeNode]) -> bool:
        ...
        # node is a valid bst node if
            # node has no bounds (root)
            # left node is strictly less than the upper_bound
            # right node is strictly more than the lower_bound
        ...
        def dfs(curr_node, lower_bound, upper_bound):
            ...
            if upper_bound is not None:
                if curr_node.val >= upper_bound:
                    return False
            if lower_bound is not None:
                if curr_node.val <= lower_bound:
                    return False
